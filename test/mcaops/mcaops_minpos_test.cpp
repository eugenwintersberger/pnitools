...
#define BOOST_TEST_MODULE acceptance test for mcaops minpos operation
...
struct mcaops_global_fixture 
{
    ...
    mcaops_global_fixture()
    {
        ...
        total_result = result_parser(read_data(total_argmin_file.string()));
        roi1_result = result_parser(read_data(roi1_argmin_file.string())); 
        roi2_result = result_parser(read_data(roi2_argmin_file.string()));
...
BOOST_FIXTURE_TEST_SUITE(mcaops_minpos,mcaops_suite_fixture)
    ...
    BOOST_AUTO_TEST_SUITE(nexus_input)
        ...
        {
            run_test({base_opt,mca_opt,"minpos","fiodata.nxs"});
        ...
        {
            run_test({roi1_opt,base_opt,mca_opt,"minpos","fiodata.nxs"});
            BOOST_CHECK_EQUAL(runner.return_value(),0);
            fix_roi1_result();
            BOOST_CHECK_EQUAL_COLLECTIONS(result.begin(),result.end(),
        ...
        {
            run_test({roi2_opt,base_opt,mca_opt,"minpos","fiodata.nxs"});
            BOOST_CHECK_EQUAL(runner.return_value(),0);
            fix_roi2_result();
            BOOST_CHECK_EQUAL_COLLECTIONS(result.begin(),result.end(),
    ...
    BOOST_AUTO_TEST_SUITE(fio_input)
        BOOST_AUTO_TEST_CASE(test_total_result)
        {
            run_test({"minpos",fio_range_1.string(),fio_range_2.string()});
            BOOST_CHECK_EQUAL(runner.return_value(),0);
            BOOST_CHECK_EQUAL_COLLECTIONS(result.begin(),result.end(),
                                          total_result.begin(),total_result.end());
        }
        ...
        BOOST_AUTO_TEST_CASE(test_roi1_result)
        {
            run_test({roi1_opt,"minpos",fio_range_1.string(),
                                     fio_range_2.string()});
            BOOST_CHECK_EQUAL(runner.return_value(),0);
            fix_roi1_result(); 
            BOOST_CHECK_EQUAL_COLLECTIONS(result.begin(),result.end(),
        ...
        BOOST_AUTO_TEST_CASE(test_roi2_result)
        {
            run_test({roi2_opt,"minpos",fio_range_1.string(),
                                      fio_range_2.string()});
            BOOST_CHECK_EQUAL(runner.return_value(),0);
            fix_roi2_result(); 
            BOOST_CHECK_EQUAL_COLLECTIONS(result.begin(),result.end(),
    ...
    BOOST_AUTO_TEST_SUITE(stdin_input)
        ...
        BOOST_AUTO_TEST_CASE(test_roi1_result)
        {
            ...
            for(auto p: stdin_files)
            {
                fs::path ifile(stdin_path);
                ifile /= p;
                run_test({roi1_opt,"minpos","<"+ifile.string()});
                BOOST_CHECK_EQUAL(runner.return_value(),0);
                BOOST_CHECK_EQUAL(result.size(),1);
        ...
        BOOST_AUTO_TEST_CASE(test_roi2_result)
        {
            ...
            for(auto p: stdin_files)
            {
                fs::path ifile(stdin_path);
                ifile /= p;
                run_test({roi2_opt,"minpos","<"+ifile.string()});
                BOOST_CHECK_EQUAL(runner.return_value(),0);
                BOOST_CHECK_EQUAL(result.size(),1);
