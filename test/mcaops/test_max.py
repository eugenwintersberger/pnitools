...
class mcaops_max_test_fio(mcaops_max_test):
    ...
    def get_total_result(self):
        result =  check_output([self.command,"max",
                                self.files1,self.files2,
                               ])
        return result

    def get_roi1_result(self):
        return check_output([self.command,"--roi=5:501","max",
                             self.files1,self.files2])

    def get_roi2_result(self):
        return check_output([self.command,"--roi=1024:2013", "max",
                             self.files1,self.files2])
...
class mcaops_max_test_stdio(mcaops_max_test):
    ...
    def get_total_result(self):
        ...
        for input_file in self.input_files:
            ...
            result += check_output([self.command,"max"],stdin=tail.stdout)
    ...
    def get_roi1_result(self):
        ...
        for input_file in self.input_files:
            tail = Popen(["tail","-n2048",input_file],stdout=PIPE)
            result += check_output([self.command,"--roi=5:501","max"],
                                    stdin=tail.stdout)
    ...
    def get_roi2_result(self):
        ...
        for input_file in self.input_files:
            tail = Popen(["tail","-n2048",input_file],stdout=PIPE)
            result += check_output([self.command,"--roi=1024:2013","max"],
                                    stdin=tail.stdout)
